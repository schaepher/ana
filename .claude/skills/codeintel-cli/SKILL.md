...
codeintel rule add "<expr>" --repo <path>    # 用户连线规则（Q220c）：值流验证不了的外键列手动声明连线
                                           #   "merchant_id → mch_merchant.id"  模式规则（所有含 merchant_id 列的表 → mch_merchant.id）
                                           #   "table_a.merchant_id → mch_merchant.id"  显式列对（单对）
                                           #   目标列省略默认 id；→ 或 -> 均可；生成关系 type=fk（ER 默认显示）
