...
codeintel query <sub> ... --repo <path>    # 查询（见下）
...
- 坑：`pkill -f "codeintel-e2e serve"` 会匹配自身命令行自杀（用 `pgrep -x codeintel-e2e` + kill）；CLI 输出可能混入 entrylog 的 OTel span JSON（`{"Name":"codeintel.main",...}`），展示时过滤
