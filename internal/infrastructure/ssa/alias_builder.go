...
type aliasPass struct {
	...
	may         map[domain.CanonicalID]map[ssa.Value]map[*ssa.Alloc]bool
	allocIDs    map[*ssa.Alloc]domain.CanonicalID
	paramMay    map[*ssa.Parameter]map[*ssa.Alloc]bool
	callMay     map[ssa.Value]map[*ssa.Alloc]bool
	excluded    map[domain.CanonicalID]map[domain.CanonicalID]map[string]bool
...
func computeAliases(repo *domain.Repository, prog *ssa.Program,
	idents map[token.Pos]string, emit domain.EmitFunc) (*aliasResult, error) {
	logger := zap.L()
	...
	for fn := range ssautil.AllFunctions(prog) {
		...
		p.funcIDs[fn] = id
		p.may[id] = map[ssa.Value]map[*ssa.Alloc]bool{}
...
func (p *aliasPass) mayOf(fn *ssa.Function, v ssa.Value) map[*ssa.Alloc]bool {
...
func (p *aliasPass) mayOfDepth(fn *ssa.Function, v ssa.Value,
	visiting map[ssa.Value]bool, depth int) map[*ssa.Alloc]bool {
...
func mergeMay(dst *map[*ssa.Alloc]bool, src map[*ssa.Alloc]bool) bool {
	if len(src) == 0 {
		return false
	}
	if *dst == nil {
		*dst = map[*ssa.Alloc]bool{}
	}
	changed := false
	for a := range src {
		if !(*dst)[a] {
...
func (p *aliasPass) emitAliasEdges(fn *ssa.Function, v ssa.Value) {
	...
	for alloc := range p.mayOf(fn, v) {
		allocID, ok := p.allocIDOf(alloc)
		if !ok {
			continue
		}
...
// allocIDOf 确保 alloc 的 ssa_value 节点发射（Q7：被别名引用的 alloc 也发射）。
func (p *aliasPass) allocIDOf(alloc *ssa.Alloc) (domain.CanonicalID, bool) {
	if id, ok := p.allocIDs[alloc]; ok {
		return id, true
	}
	fn := alloc.Parent()
	if fn == nil {
	...
	slot := alloc.Name()
	if slots[slot] {
		line := p.prog.Fset.PositionFor(alloc.Pos(), false).Line
