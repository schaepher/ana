...
- **方法 receiver 连线**（2026-08-13）：展开方法时显示到其接收者类型
  节点的连线——AST 适配器（emitMethodReceiver）为每个带 receiver 的
  方法声明建立 has_receiver 边（方法 → 接收者类型，conf 0.8），接收者
  类型节点如不存在则创建（轻量节点，SCIP 已建则 UPSERT 合并）；Expand
  查询与前端边样式（虚线 [5,2]、标注"接收者"）、信息栏分组均已加入。
  **布局坑**：中间行只有单个节点（如 receiver）时 placeRow 会把它放在
  中心节点正上方导致重叠——单个 mid 节点须偏移到中心右侧（offsetSingle）。
